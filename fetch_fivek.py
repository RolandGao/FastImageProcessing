...
def download(url, fn=None, path=None):
    ...
    with open(dest_fn, 'wb') as f:
        ...
        while True:
            ...
            progress = '  {:.2f}MB  [{:3.2f}%]'.format(downloaded / 1024 ** 2,
...
if __name__ == '__main__':
    print(
        'This file downloads ready-to-use packages of the Adobe-MIT FiveK dataset.')
    ...
    print('# File 2/3')
    download(fn_template.format('image_raw.npy'),
             path='data/fivek_dataset/sup_batched80aug_daylight')

    print('# File 3/3')
    download(fn_template.format('meta_raw.pkl'),
             path='data/fivek_dataset/sup_batched80aug_daylight')
