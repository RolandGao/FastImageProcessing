...
import L0_smoothing.L0_helpers as L0_helpers
...
def l0_smoothing(image, kappa=2.0, _lambda=2e-2):
    # Timers
    step_1 = 0.0
    step_2 = 0.0
    step_2_fft = 0.0
...
if __name__ == '__main__':
    ...
    #     smoothed_image=l0_smoothing(image)
    #     cv2.imwrite(out_name, smoothed_image)
    input_folder = "../data/clustering_images3"
    output_folder = "clustering_images3_smoothed"
    os.makedirs(output_folder,exist_ok=True)
    for i, filename in enumerate(sorted(os.listdir(input_folder))):
        print(i)
        in_name = os.path.join(input_folder, filename)
        ...
        smoothed_image=l0_smoothing(image,_lambda=0.004)
        cv2.imwrite(out_name, smoothed_image)

    # # print(os.path.exists(image_r))
    # image = cv2.imread("flower.tiff")
    # image=l0_smoothing(image)
    # cv2.imwrite("smoothed_flower.tiff",image)
