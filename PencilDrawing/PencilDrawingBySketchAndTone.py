...
if __name__ =='__main__':
    # import os
    # input_folder = "../data/artists/FiveK_C"
    # input_folder='inputs'
    # output_folder = "pencil_drawing"
    # pencil_tex = './pencils/pencil1.jpg'
    # os.makedirs(output_folder, exist_ok=True)
    # for i, filename in enumerate(sorted(os.listdir(input_folder))):
    #     print(i)
    #     #filename='11--128.jpg'
    #     in_name = os.path.join(input_folder, filename)
    #     out_name = os.path.join(output_folder, filename)
    #     image = io.imread(in_name)
    #     pencil_image = gen_pencil_drawing(image, kernel_size=8, stroke_width=0,
    #                                    num_of_directions=8,
    #                                    smooth_kernel="gauss",
    #                                    gradient_method=0, rgb=True, w_group=2,
    #                                    pencil_texture_path=pencil_tex,
    #                                    stroke_darkness=2, tone_darkness=1.5)
    #     pencil_image=(pencil_image*255).clip(0,255).astype("uint8")
    #     io.imsave(out_name,pencil_image)
    #
    #     if i==5:
    #         break
    ...
    #ex_img = io.imread('./inputs/flower.png')
    #'11--128.jpg'
    ex_img=Image.open('./inputs/11--128.jpg').convert('RGB')
    pencil_tex = './pencils/pencil3.jpg'
    ex_im_pen = gen_pencil_drawing(ex_img, kernel_size=26, stroke_width=2,
                                   num_of_directions=8, smooth_kernel="median",
                                   gradient_method=1, rgb=True, w_group=1,
                                   pencil_texture_path=pencil_tex,
                                   stroke_darkness=2, tone_darkness=1.5)
